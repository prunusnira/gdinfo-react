...
interface Props {
    ...
    tableTxtDesc: string,
    skillPageVersion: (ptype: number) => string,
                ...
                <ItemRow>
                    {
                        ...
                        (
                            <Link style={{width:"100%"}} to={`/skill/${props.ptype}/${props.userid}/${props.gtype}/${props.page}/${props.order}${window.location.search}`}>
                                <Button style={{width:"100%"}}>
                                    {(txtSkill.btnNormalTable as any)[props.lang]}
                                </Button>
                            </Link>
                        )
                        :
                        (
                            <Link style={{width:"100%"}} to={`/skillscr/${props.ptype}/${props.userid}/${props.gtype}/${props.page}/${props.order}${window.location.search}`}>
                                <Button style={{width:"100%"}}>
                                    {(txtSkill.btnShareTable as any)[props.lang]}
                                </Button>
                            </Link>
                        )
                    }
                    ...
                    <Button style={{width:"100%"}} onClick={() => props.scrShot("scrTable", `${props.userid}_${props.gtype}_all_${props.page}_${Time.getTimeScr()}.jpg`)}>
                        {(txtSkill.scrshot as any)[props.lang]}
                    </Button>
                </ItemRow>
                {
                            ...
                            return (
                                <ItemRow>
                                    <Button style={{width:"100%"}} onClick={() => props.createSnapshot(props.userid, props.gtype)}>
                                        {(txtSkill.snapshot.button as any)[props.lang]}
                                    </Button>
                                </ItemRow>
                            )
                        }
                    ...
                    return (
                        <ItemRow setVertical={true}>
                            ...
                            <BodyContent>
                                ...
                                <ItemRow>
                                    <Button onClick={() => props.switchOrder(0)}>{(txtSkill.filter.btn.title as any)[props.lang]} ▲/▼</Button>
                                    <Button onClick={() => props.switchOrder(1)}>{(txtSkill.filter.btn.version as any)[props.lang]} ▲/▼</Button>
                                </ItemRow>
                            </BodyContent>
                        </ItemRow>
                    )
        ...
        <ItemRow id='scrTable' setVertical={true}>
            <BodyHeader>
                <ItemRow id="targetInfo" setVertical={true}>
                    <ItemRow style={{justifyContent: 'center'}}>
                        <h4><b>GITADORA {props.skillPageVersion(parseInt(props.ptype))}<br/>
            ...
            <BodyContent>
                <ItemRow className="skillupper blackandwhite">
                    {
                        (function() {
                            if(props.ptype === ('3' || '5' || '7' || '9' || '11')) {
                                return (
                                    <ItemRow>
                                        <span style={{textAlign:"center"}}>
                                            * {(txtSkill.oldver as any)[props.lang]}
                                        </span>
                                    </ItemRow>
                                )
                            }
