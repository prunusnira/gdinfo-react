...
const SingleSkillColorChanger = (props: Props) => {
    ...
    if (props.skill !== undefined) {
        return (
            <Link className={color(props.skill)} to={props.link}>
                <span style={{ fontWeight: 'bold' }}>{props.skill.toFixed(2)}</span>
            </Link>
        );
    }
    return (
        <Link className={color(0)} to={props.link}>
...
export const skillTableColorFlat = (skill: number) => {
    ...
    else if (skill < 180000000)
        color = 'background: linear-gradient(to right, #FF0000, #FF8C00, #FFFF00, #80FF00)';
    else if (skill < 190000000)
        color = 'background: linear-gradient(to right, #80FF00, #58ACFA, #0101DF)';
