...
const useSkillRankingData = ({ gtype, page }: Props) => {
    ...
    const updateRankList = () => {
        if (gtype && page) {
            return getSkillRank(gtype, page);
        }
        return undefined;
    };

    const {data, isLoading} = useQuery({
        queryKey: ['skill', 'ranking', gtype, page],
        queryFn: updateRankList,
    })

    useEffect(() => {
        if(data && page) {
            const list = new Array<ISkillRank>();
            const userList = JSON.parse(data.allUserList);
    ...
    return { rankList, allPage, isLoading };
};
