export const Black = "#1f1f1f";
export const White = "#ffffff";
export const BGGray = "#2f2f2f";
export const SkillTitleColor = "#1676de";
export const SkillBottomLine = "#5f5f5f";

export const SeaBlue = "rgb(0, 197, 212)";

export const MenuBack = "rgb(255, 255, 255, 0.9)";
export const MenuBackDark = "rgb(74, 74, 74, 0.9)";
export const ContentTitle = "#ffa952";
export const ContentBody = "#feffdf";
export const ContentBodyDark = "#4f4f4f";
export const Background = "#fff"; //"#ef5a5a";

export const SkillBGColor = Black;

export const HeaderBGColor = "rgba(240, 229, 180, 0.8)";
export const HeaderBGColorDark = "rgba(12, 65, 151, 0.8)";

export const AnchorColor = "rgb(12, 65, 151)";
export const AnchorColorDark = "rgb(242, 240, 96)";

export const Select = "#A6E7D6";
export const SelectDark = "rgb(12, 65, 151)";

export const ButtonBG = "#f9cba2";
