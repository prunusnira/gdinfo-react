...
export const getNoRecordData = async (
    ...
    const res =
        await axios.get(`${CommonData.dataUrl}notplayed/${gtype}/${userid}/${vertype}/${page}${search}`)
    return res.data
}
...
export const getPatternList = async (
    ...
    const res =
        await axios.get(`${CommonData.dataUrl}pattern/${ver}/${order}/${page}${search}`)
    return res.data
}
...
export const getPatternRank = async (
    ...
    const res =
        await axios.post(`${CommonData.dataUrl}ptrank/${mid}/${ptcode}/${page}/${verparam}`)
    return res.data
}